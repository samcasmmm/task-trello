...
export default function TaskTimeline({ tasks = [] }: TaskTimelineProps) {
    ...
    return (
      ...
      >
        <CardContent className='p-6'>
          ...
          <h3 className='text-sm font-bold mb-1 text-slate-300'>
            No Tasks with Dates
          </h3>
  ...
  const timeValues = datedTasks.flatMap((t) => {
    ...
    if (t.due_date || t.dueDate)
      dates.push(new Date(t.due_date || t.dueDate).getTime())
    return dates
  })
    ...
    >
      <CardContent className='p-4 overflow-x-auto'>
        <div className='min-w-[800px] space-y-4'>
          ...
          <div className='flex items-center text-[10px] font-bold uppercase tracking-wider text-slate-500 pb-2 border-b' style={{ borderColor: 'var(--border-subtle)' }}>
            ...
            {datedTasks.map((t) => {
              ...
              return (
                <div key={t.id} className='flex items-center text-xs'>
                  ...
                  <div className='w-3/4 relative h-7 border rounded-md overflow-hidden' style={{ background: 'var(--surface-1)', borderColor: 'var(--border-subtle)' }}>
                    <div
                      className='absolute top-1 bottom-1 rounded border transition-all flex items-center px-2 overflow-hidden'
                      style={{
                      ...
                      title={`${t.title}: ${formatDate(start)} to ${formatDate(end)}`}
                    >
