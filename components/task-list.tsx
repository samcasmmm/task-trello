...
export default function TaskList({ tasks = [] }: { projectId: string; tasks: any[] }) {
  ...
  const sorted = [...filtered].sort((a, b) => {
    const aV = sortField === 'assigned_to' ? a.assigned_to_user?.fullName || '' : a[sortField];
    const bV = sortField === 'assigned_to' ? b.assigned_to_user?.fullName || '' : b[sortField];
                ...
                sorted.map((task, i) => {
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`text-[10px] font-bold px-2 py-1 rounded capitalize ${STATUS_CLASSES[task.status] || STATUS_CLASSES.todo}`}
                        >
