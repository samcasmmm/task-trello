...
async function seed() {
  ...
  const permissionData = [
    ...
    { id: 'p-manage-tenants', name: 'manage_tenants', description: 'Manage tenants/workspaces' },
    {
      ...
      description: 'View all projects across platform',
    },
    {
      id: 'p-manage-projects',
      name: 'manage_projects',
      description: 'Create and edit workspace projects',
    },
  ...
  const roleData = [
    {
      id: 'r-super-admin',
      name: 'super_admin',
      description: 'Platform owner with full global control',
    },
    {
      id: 'r-tenant-manager',
      name: 'tenant_manager',
      description: 'Workspace owner who manages projects and teams',
    },
  ...
  const tenantManagerPerms = ['p-manage-projects', 'p-manage-tasks', 'p-manage-fields'];
  const memberPerms = [
  ...
  const projectData = [
    {
      ...
      description:
        'Completely overhaul our web application interface and user experience to drive product conversion',
    ...
    },
    {
      ...
      description:
        'Prepare launch resources, content strategy, and SEO optimization for next-gen products',
      color: '#EC4899', // Pink
      icon: 'trending-up',
      status: 'active',
      createdById: 'u-manager',
  ...
  await db
    ...
    .values({
      ...
      description:
        'Overhaul design systems, establish component guidelines, and implement key structural components.',
      status: 'in_progress',
      priority: 'high',
  ...
  await db
    ...
    .values({
      ...
      description:
        'Vibrant modern header with dark mode toggle, engaging CTA buttons, and interactive glassmorphic card display.',
      status: 'done',
      priority: 'urgent',
      assignedToId: 'u-member',
      createdById: 'u-manager',
  ...
  await db
    ...
    .values({
      ...
      description:
        'Initialize Next.js project with Tailwind CSS, shadcn/ui library, and configure standard layouts.',
      status: 'todo',
      priority: 'high',
      assignedToId: 'u-member',
      createdById: 'u-manager',
  ...
  await db
    ...
    .values({
      ...
      description:
        'Ensure secure Neon DB connections, deploy production indexes, and run automated stress test queries.',
      status: 'todo',
      priority: 'high',
      assignedToId: 'u-manager',
      createdById: 'u-manager',
  ...
  await db
    ...
    .values({
      ...
      description:
        'Connect Google Analytics and set up Vercel dashboard metrics for campaign funnel clicks.',
      status: 'done',
      priority: 'low',
      assignedToId: 'u-member',
      createdById: 'u-manager',
  ...
  await db
    .insert(taskMembers)
    .values({ id: crypto.randomUUID(), taskId: 't1-root', userId: 'u-manager' })
    .execute();
  await db
    .insert(taskMembers)
    .values({ id: crypto.randomUUID(), taskId: 't1-root', userId: 'u-member' })
    .execute();
  await db
    .insert(taskMembers)
    .values({ id: crypto.randomUUID(), taskId: 't1-child1', userId: 'u-member' })
    .execute();
  ...
  await db
    .insert(taskLabels)
    .values({ id: crypto.randomUUID(), taskId: 't1-root', labelId: 'l-design' })
    .execute();
  await db
    .insert(taskLabels)
    .values({ id: crypto.randomUUID(), taskId: 't1-child1', labelId: 'l-design' })
    .execute();
  await db
    .insert(taskLabels)
    .values({ id: crypto.randomUUID(), taskId: 't1-sub1', labelId: 'l-feat' })
    .execute();
  await db
    .insert(taskLabels)
    .values({ id: crypto.randomUUID(), taskId: 't2-root', labelId: 'l-bug' })
    .execute();
  ...
  const checklistData = [
    {
      id: crypto.randomUUID(),
      taskId: 't1-root',
      title: 'Conduct user research study',
      completed: true,
      orderIndex: 0,
    },
    {
      id: crypto.randomUUID(),
      taskId: 't1-root',
      title: 'Prepare wireframe layout drafts',
      completed: true,
      orderIndex: 1,
    },
    {
      id: crypto.randomUUID(),
      taskId: 't1-root',
      title: 'Develop front-end component assets',
      completed: false,
      orderIndex: 2,
    },
    {
      id: crypto.randomUUID(),
      taskId: 't1-root',
      title: 'Run cross-browser responsiveness tests',
      completed: false,
      orderIndex: 3,
    },
  ];
  ...
  await db
    ...
    .values({
      ...
      userId: 'u-manager',
      content:
        'Awesome work. Make sure the buttons have rounded-lg corners to align with our branding design.',
  ...
  const auditData = [
    {
      ...
      userId: 'u-admin',
      action: 'user_created',
      metadata: { email: 'manager@tenant.com', fullName: 'SaaS Manager' },
    },
    {
      id: crypto.randomUUID(),
      userId: 'u-admin',
      action: 'user_created',
      metadata: { email: 'member@tenant.com', fullName: 'Regular Dev' },
    },
    {
      id: crypto.randomUUID(),
      userId: 'u-manager',
      action: 'tenant_created',
      metadata: { tenantId: 't-acme', name: 'Acme Software Solutions' },
    },
    {
      id: crypto.randomUUID(),
      userId: 'u-manager',
      action: 'project_created',
      metadata: { projectId: 'p-redesign', name: 'SaaS Platform Redesign' },
    },
    {
      id: crypto.randomUUID(),
      userId: 'u-manager',
      action: 'task_created',
      metadata: { taskId: 't1-root', title: 'Website Redesign UX Overhaul' },
    },
    {
      id: crypto.randomUUID(),
      userId: 'u-member',
      action: 'task_status_updated',
      metadata: { taskId: 't1-sub1', oldStatus: 'todo', newStatus: 'done' },
    },
    {
      id: crypto.randomUUID(),
      userId: 'u-member',
      action: 'comment_added',
      metadata: { taskId: 't1-root', commentLength: 61 },
    },
  ];
  ...
  await db
    ...
    .values({
      ...
      userId: 'u-member',
      content:
        'You have been assigned to task: "Design Responsive Dropdown Navbar" by SaaS Manager.',
      read: false,
    })
    .execute();
