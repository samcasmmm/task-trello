...
export async function GET(
  ...
  try {
    ...
    const [aggregateRows, statusRows, priorityRows, createdMonthRows, completedMonthRows] = await Promise.all([
      // KPI aggregate
      db
        .select({
    ...
    const tasksByPriority = priorityRows.map((r) => ({ priority: r.priority, count: Number(r.count) }))
