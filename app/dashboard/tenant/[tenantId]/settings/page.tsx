      ...
      <div key={userId} className="space-y-4 relative pl-5 border-l border-border-subtle mt-4">
        ...
        <div className="flex items-center gap-3 p-3 bg-surface-2 border border-border-subtle hover:border-border-strong transition-colors rounded-lg shadow-md max-w-md">
          <Avatar className="h-9 w-9 border border-border-subtle">
            ...
            <AvatarFallback className="bg-surface-3 text-slate-200 font-bold text-[10px]">
          ...
          <div className="flex-1 min-w-0">
            <h4 className="text-xs font-bold text-foreground flex items-center gap-2">
              ...
              <Badge className="text-[8px] px-1.5 py-0 capitalize bg-surface-3 text-slate-300 border border-slate-700 shadow-none font-bold">
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-border-subtle pb-5">
        <div>
          ...
          <p className="mt-1 text-foreground-dim font-semibold">{tenant.name}</p>
        </div>
      </div>

      {/* Settings Tabs Panel */}
      <Tabs defaultValue="members" className="w-full">
        <TabsList className="bg-surface-2 p-1 border border-border-subtle rounded-lg w-full sm:w-auto grid grid-cols-3 gap-1 mb-6">
          <TabsTrigger
            value="members"
            className="gap-2 text-xs font-semibold px-3 py-1.5 rounded-md data-[state=active]:bg-surface-3"
          >
          ...
          <Card className="rounded-lg overflow-hidden bg-surface-2 border border-border-subtle shadow-none">
            <CardHeader className="pb-3 border-b bg-surface-1 border-border-subtle flex flex-row items-center justify-between">
              ...
              <Dialog open={showRoleDialog} onOpenChange={setShowRoleDialog}>
                ...
                <DialogContent className="max-w-xl bg-slate-900 border border-slate-800 text-slate-100 shadow-none rounded">
                  <DialogHeader>
                    ...
                    <div className="space-y-2 border-t border-slate-800 pt-3">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-48 overflow-y-auto pr-1">
                        {permissionsList.map((perm) => (
                          <div
                            key={perm.id}
                            className="flex items-start gap-2 p-2 bg-slate-950/40 rounded border border-slate-800 hover:bg-slate-900/50 transition-colors"
                          >
                            <Checkbox
                              id={perm.id}
                            ...
                            <div className="grid gap-0.5 leading-none">
                              <label
                                htmlFor={perm.id}
                                className="text-xs font-bold text-slate-300 cursor-pointer capitalize"
                              >
            ...
            <CardContent className="pt-6">
              <div className="grid gap-4 md:grid-cols-2">
                {rolesList.map((role) => (
                  ...
                  >
                    <div
                      className={`absolute left-0 top-0 h-full w-1 ${role.tenantId ? 'bg-amber-500' : 'bg-slate-700'}`}
                    />
                    <CardHeader className="pb-2">
                      <div className="flex items-center justify-between">
                        <CardTitle className="text-sm font-extrabold capitalize text-slate-200 flex items-center gap-1.5">
                          ...
                          {role.tenantId ? (
                            ...
                            <Badge className="bg-slate-800 text-slate-300 border border-slate-700 shadow-none text-[8px] py-0">
                              System Role
                            </Badge>
                          )}
                        </CardTitle>
                          ...
                          {role.permissions && role.permissions.length > 0 ? (
                            role.permissions.map((p) => (
                              <Badge
                                key={p}
                                variant="secondary"
                                className="text-[9px] capitalize shadow-none bg-surface-2 border border-border-subtle text-slate-400"
                              >
                                {p.replace(/_/g, ' ')}
                              </Badge>
                            ))
                          ) : (
                            <span className="text-slate-500 italic">No direct permissions.</span>
                          )}
                        </div>
                      </div>
                    </CardContent>
                      ...
                      <div className="flex items-center gap-3 p-3 bg-surface-3 border border-border-strong text-white rounded-lg shadow-md max-w-md">
                        ...
                        <div className="flex-1">
                          <h4 className="text-sm font-extrabold flex items-center gap-1.5">
                            ...
                            <Badge className="text-[8px] bg-slate-850 border border-slate-700 text-slate-200 font-mono shadow-none uppercase font-bold py-0.5 px-1.5">
                              Head
                            </Badge>
                          </h4>
